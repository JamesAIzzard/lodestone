/**
 * Edit tool registration for file editing operations.
 */
...
// â”€â”€ Target resolution helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

interface ResolvedTarget {
  filePath: string;
  puidKey?: string;
  puidRecord?: PuidRecord;
  isDirectory: boolean;
}
...
// â”€â”€ Per-operation handlers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function handleCreate(
  ...
  full_document: boolean | undefined,
...
async function handleMove(
  ...
  destination_type: 'directory' | 'filepath' | undefined,
  on_conflict: 'error' | 'skip' | 'overwrite' | undefined,
  dry_run: boolean | undefined,
): Promise<CallToolResult> {
  if (target === undefined) {
  ...
  // Staleness check (files only â€” directories don't have content hashes)
  const staleMsg = checkStaleness(filePath, puidRecord);
  if (staleMsg) {
    return textResponse(staleMsg);
  }
...
async function handleBatchMove(
  ...
  targets: string[],
  destination: string,
  destination_type: 'directory' | 'filepath',
  on_conflict: 'error' | 'skip' | 'overwrite' | undefined,
  dry_run: boolean | undefined,
): Promise<CallToolResult> {
  // Resolve destination (d-puid or path) â€” shared across all batch elements
  const destResult = resolveDirPuid(destination, ctx.puid);
  if (typeof destResult !== 'string') return destResult;
  ...
  for (const element of targets) {
    ...
    if (staleMsg) {
      batchResults.push({
        source: filePath,
        success: false,
        error: 'File has been modified externally since last read.',
      });
      continue;
    }
    ...
    batchResults.push({
      ...
      conflict: result.conflict,
      destListing: result.destinationDirectoryListing,
    });
  ...
  // Single destination directory listing (collapsed format â€” from last successful move)
  ...
  if (skipped > 0)
    summaryParts.push(`${skipped} ${on_conflict === 'skip' ? 'skipped' : 'conflicted'}`);
...
async function handleBatchDelete(
  ...
  parts.push('');
  parts.push(
    `${succeeded} of ${batchResults.length} operations succeeded${failed > 0 ? `, ${failed} failed` : ''}.`,
  );
  return textResponse(parts.join('\n'));
}
...
async function handleTextEdit(
  ...
  if (/^r\d+$/.test(target)) {
    ...
    if (!resolved) {
      return errorResponse(
        `Unknown puid "${target}". It may be from a previous session. Search or explore again to get a fresh reference.`,
      );
    }
    if ('error' in resolved) {
  ...
  // 2. Staleness check (puid path only â€” raw filepaths skip this)
  if (puidRecord?.contentHash) {
    ...
    if (currentHash !== puidRecord.contentHash) {
      ...
      try {
        ...
        /* if read fails, just show hashes */
      }
      puidRecord.contentHash = currentHash;
...
// â”€â”€ Tool registration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  server.tool(
    ...
    EDIT_DESCRIPTION,
    {
      ...
      target: z
        ...
        .optional()
        .describe(
          'Puid (e.g. "r3", "d5") or absolute path. Required for str_replace, insert_at_line, overwrite, append, rename, move, delete. move and delete accept an array for batch operations.',
        ),
      old_str: z
        .string()
        .optional()
        .describe(
          'String to find and replace (must match exactly once). Required for str_replace.',
        ),
      ...
      line: z
        ...
        .min(1)
        .optional()
        .describe('1-based line number to insert before. Required for insert_at_line.'),
      content: z
        .string()
        .optional()
        .describe(
          'Content to insert/write/append/create. Required for insert_at_line, overwrite, append, create.',
        ),
      ...
      context_lines: z
        ...
        .min(0)
        .optional()
        .describe('Lines of surrounding context in confirmation (default: from config).'),
      full_document: z
        .boolean()
        .optional()
        .describe('Return the full document after edit instead of a context snippet.'),
      directory: z
        .string()
        .optional()
        .describe('D-puid (e.g. "d5") or absolute directory path. Required for create and mkdir.'),
      filename: z.string().optional().describe('Name of the file to create. Required for create.'),
      name: z
        .string()
        .optional()
        .describe(
          'Name for mkdir (new directory) or rename (new name). Required for mkdir and rename.',
        ),
      destination: z
        .string()
        .optional()
        .describe('D-puid, absolute directory path, or absolute filepath. Required for move.'),
      destination_type: z
        .enum(['directory', 'filepath'])
        .optional()
        .describe(
          'Whether destination is a directory (preserve filename) or a filepath (rename). Required for move.',
        ),
      on_conflict: z
        .enum(['error', 'skip', 'overwrite'])
        .optional()
        .describe(
          'Conflict resolution for move when destination exists. "error" (default) rejects, "skip" silently skips, "overwrite" replaces. Only applies to file targets.',
        ),
    },
    async ({
      operation,
