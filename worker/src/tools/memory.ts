...
export function registerTaskTool(server: McpServer, memory: D1MemoryService): void {
  server.tool(
    ...
    [
      ...
      '  priority     \u2014 1=low, 2=medium, 3=high. Default: 2 (medium).',
    ...
    ].join('\n'),
    {
      ...
      priority: z.number().int().min(1).max(3).optional().describe('Urgency: 1=low, 2=medium, 3=high. Default: 2 (medium)'),
    ...
    withErrorHandling(async ({ topic, body, action_date, due_date, recurrence, priority, project, force }) => {
      ...
      const result = await memory.remember({
        ...
        priority: ((priority as number | undefined) ?? 2) as PriorityLevel,
        status: 'open',
        completedOn: null,
        projectId,
      });
